...
import { EditCustomer, customerList, editBanner } from '../../utils/api.services';
import { ExportToCSV, ExportToExcel, ExportToPdf, TOAST_ERROR, TOAST_SUCCESS } from '../../config/common';
import { DataTable } from 'primereact/datatable';
import { Column } from 'primereact/column';
import "primereact/resources/themes/lara-light-cyan/theme.css";
import { getBannerListThunk, getUserListThunk, setLoader, updateBannerList } from '../../Store/slices/MasterSlice';
...
export default function ManageBanner() {
    ...
    const { bannerList: { data: bannerList } } = useSelector((state) => state.masterslice);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                await dispatch(getBannerListThunk(request));
            } finally {
    ...
    const handleDelete = (is_true) => {
        if (is_true) {
            // dispatch(setLoader(true));
            let submitData = {
                banner_id: selectedUser?.id,
                is_delete: '1',
            }
            editBanner(submitData).then((response) => {
                if (response.code === Codes?.SUCCESS) {
                    const updatedList = bannerList?.result?.filter((item) => item._id !== selectedUser?._id);
                    dispatch(updateBannerList({
                        ...bannerList,
                        result: updatedList
                    }))
                    closeModel(dispatch)
    ...
    return (
        <>
            <div className="container-fluid mw-100">
                <SubNavbar title={"Banner List"} header={'Banner List'} />
                <div className="widget-content searchable-container list">
                    <div className="card card-body">
                        <div className="row">
                            <div className="col-md-4 col-xl-3">
                                <div className="position-relative">
                                    <input type="text" className="form-control product-search ps-5" id="input-search" placeholder="Search Banner..."
                            ...
                            <div className="col-md-8 col-xl-9 text-end d-flex justify-content-md-end justify-content-center mt-3 mt-md-0">
                                ...
                                <div id="btn-add-contact" className="btn btn-info d-flex align-items-center" style={{ height: '40px' }} onClick={() => { navigat(PATHS?.BANNER_ADD) }}>
                                    <i className="ti ti-category me-1 fs-6" />Add Banner
                                </div>
                            </div>
                            {/* {/* <div className="col-md-8 col-xl-9 text-end d-flex justify-content-md-end justify-content-center mt-3 mt-md-0"> */}
                            ...
                            <div className="col-md-8 col-xl-9 text-end d-flex justify-content-md-end justify-content-center mt-3 mt-md-0">
                                ...
                                {/* <Link to={'/customer_list/add_customer'} id="btn-add-contact" className="btn btn-info d-flex align-items-center" style={{ height: '40px' }}>
                                    <i className="ti ti-category me-1 fs-6" />Add Customer
                                </Link> */}
                    ...
                    <div className="card card-body">
                        <div className="table-responsive">
                            <DataTable
                                ...
                                loading={loading}
                                emptyMessage={<span style={{ textAlign: 'center', display: 'block' }}>No Banner found.</span>}
                            >
                                <Column
                                    field="id"
                                    header="Id"
                                    ...
                                    body={(rowData) => (
                                        <div style={{ display: 'flex', justifyContent: 'start', alignItems: 'start' }}>
                                            <img
                                                ...
                                                alt="Profile"
                                                height={50}
                                                width={80} // Ensures a consistent aspect ratio
                                                style={{
                                                    objectFit: 'cover',
                                                    borderRadius: '10px' // Ensures proper cropping
                                                }}
                                            />
                                        </div>
                                    )}
                                ...
                                <Column field="name" sortable header="Name" style={{ minWidth: '12rem', whiteSpace: 'nowrap' }} body={(rowData) => (
                                    <span>{truncateWords(rowData.name) || '-'}</span>
                                )} />

                                <Column field="start_date" sortable header="Start Date" style={{ minWidth: '12rem', whiteSpace: 'nowrap' }} body={(rowData) => (
                                    <span>{truncateWords(rowData.start_date) || '-'}</span>
                                )} />

                                <Column field="end_date" sortable header="End Date" style={{ minWidth: '12rem', whiteSpace: 'nowrap' }} body={(rowData) => (
                                    <span>{truncateWords(rowData.end_date) || '-'}</span>
                                )} />

                                <Column field="start_time" sortable header="Start Time" style={{ minWidth: '12rem', whiteSpace: 'nowrap' }} body={(rowData) => (
                                    <span>{truncateWords(rowData.start_time) || '-'}</span>
                                )} />

                                <Column field="end_time" sortable header="End Time" style={{ minWidth: '12rem', whiteSpace: 'nowrap' }} body={(rowData) => (
                                    <span>{truncateWords(rowData.end_time) || '-'}</span>
                                )} />

                                <Column field="status" header="Action" style={{ minWidth: '10rem' }} body={(rowData) => (
                                    <div className="action-btn">
                                        <a className="text-info edit cursor_pointer cursor_pointer me-2" onClick={() => navigat(PATHS?.BANNER_EDIT, { state: rowData })} >
                                            <TbEdit style={{ fontSize: '1.2rem' }} />
                                        </a>
                                        <Link to={PATHS?.BANNER_DETAILS} state={rowData} className="text-info edit cursor_pointer">
                            ...
                            <div className=''>
                                <Pagination per_page={bannerList?.per_page} pageCount={bannerList?.total} onPageChange={onPageChange} page={bannerList?.page} />
                            </div>
