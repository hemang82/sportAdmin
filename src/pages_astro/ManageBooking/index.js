...
import { EditCustomer, customerList } from '../../utils/api.services';
import { ExportToCSV, ExportToExcel, ExportToPdf, TOAST_ERROR, TOAST_SUCCESS } from '../../config/common';
import { DataTable } from 'primereact/datatable';
import { Column } from 'primereact/column';
import "primereact/resources/themes/lara-light-cyan/theme.css";
import { getbookingListThunk, getUserListThunk, setLoader, updateCustomerList } from '../../Store/slices/MasterSlice';
import Constatnt, { Codes, ModelName, SEARCH_DELAY } from '../../config/constant';
import useDebounce from '../hooks/useDebounce';
import { closeModel, formatDate, formatIndianPrice, openModel } from '../../config/commonFunction';
...
export default function ManageBooking() {
    ...
    const { bookingList: { data: userList } } = useSelector((state) => state.masterslice);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                await dispatch(getbookingListThunk(request));
            } finally {
    ...
    const handleDelete = (is_true) => {
        if (is_true) {
            ...
            // EditCustomer(submitData).then((response) => {
            //     if (response.code === Codes?.SUCCESS) {
            //         const updatedList = userList?.customerList?.filter((item) => item._id !== selectedUser?._id);
            //         dispatch(updateCustomerList({
            //             ...userList,
            //             customerList: updatedList
            //         }))
    ...
    return (
        <>
            <div className="container-fluid mw-100">
                <SubNavbar title={"Booking List"} header={'Booking List'} />
                        ...
                        <div className="row">
                            ...
                            <div className="col-md-4 col-xl-3">
                                <div className="position-relative">
                                    <input type="text" className="form-control product-search ps-5" id="input-search" placeholder="Search Booking List..."
                    ...
                    <div className="card card-body">
                        <div className="table-responsive">
                            <DataTable
                                ...
                                loading={loading}
                                emptyMessage={<span style={{ textAlign: 'center', display: 'block' }}>No Booking found.</span>}
                            >
                                <Column
                                    field="id"
                                    header="Id"
                                ...
                                <Column field="order_id" header="Order ID" style={{ minWidth: '12rem', whiteSpace: 'nowrap' }} body={(rowData) => (
                                    <span>{rowData.order_id || '-'}</span>

                                )} />

                                <Column field="slot_date" sortable header="Date" style={{ minWidth: '8rem' }} body={(rowData) => (
                                    <span>{rowData.slot_date || '-'} </span>
                                )} />

                                <Column field="venue_name" sortable header="Venue Name" style={{ minWidth: '12rem' }} body={(rowData) => (
                                    <span>{rowData.venue_name || '-'}</span>
                                )} />

                                <Column field="game_name" sortable header="Game" style={{ minWidth: '10rem' }} body={(rowData) => (
                                    <span>{rowData.game_name || '-'}</span>
                                )} />


                                <Column field="booking_slots" sortable header="Booking slots" style={{ minWidth: '10rem' }} body={(rowData) => (
                                    <span>{rowData.booking_slots || '-'}</span>
                                )} />

                                <Column field="price" sortable header="Price" style={{ minWidth: '8rem' }} body={(rowData) => (
                                    <span>{formatIndianPrice(rowData?.price) || '-'}</span>
                                )} />

                                <Column field="payment_status" sortable header="Payment Status" style={{ minWidth: '12rem' }} body={(rowData) => (
                                ...
                                <Column field="status" header="Action" style={{ minWidth: '10rem' }} body={(rowData) => (
                                    <div className="action-btn">
                                        {/* <a className="text-info edit cursor_pointer cursor_pointer me-2" onClick={() => navigat(`/customer_list/edit_customer`, { state: rowData })} >
                                            <TbEdit style={{ fontSize: '1.2rem' }} />
                                        </a> */}
                                        <Link to={PATHS?.BOOKING_DETAILS} state={rowData} className="text-info edit cursor_pointer">
                            ...
                            <div className=''>
                                <Pagination per_page={perPage} pageCount={userList?.total} onPageChange={onPageChange} page={page} />
                            </div>
