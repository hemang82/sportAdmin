...
import { editUserDetials, getUserList, editCustomerDetails, exportCustomerList } from '../../utils/api.services';
...
export default function Payment() {
    ...
    const handleStatus = async (id, changeChecked) => {
        ...
        editCustomerDetails(submitData).then((response) => {
    ...
    const handleExportToPdfManage = async () => {
        ...
        if (code === Codes.SUCCESS) {
            ...
            ExportToPdf(data, 'Payment List', 'Payment List');
        }
        dispatch(setLoader(false));
    }
    ...
    return (
        <>
            ...
            <div className="container-fluid mw-100">
                ...
                <SubNavbar title={"Payment List"} header={'Payment List'} />
                        ...
                        <div className="row">
                            <div className="col-md-4 col-xl-3">
                                <div className="position-relative">
                                    <input type="text" className="form-control product-search ps-5" id="input-search" placeholder="Search Payment..."
                    ...
                    <div className="card card-body">
                        <div className="table-responsive">
                            <DataTable
                                ...
                                loading={loading}
                                emptyMessage={<span style={{ textAlign: 'center', display: 'block' }}>No Payment found.</span>}
                            ...
                            >
                                ...
                                {/* <Column field="profile_image" header="Profile Image" style={{ minWidth: '8rem', whiteSpace: 'nowrap' }} body={(rowData) => (
                                    <img src={`https://eveksia.s3.me-central-1.amazonaws.com/profile_image/1721286162134.png`} className='ms-3 rounded-circle  ' alt="Profile" style={{ alignSelf: 'center', height: '40px', width: '40px' }} />
                                )} /> */}

                                <Column field="" sortable header="Payment Id" style={{ minWidth: '10rem', whiteSpace: 'nowrap', textTransform: 'capitalize' }} body={(rowData) => (
                                    <span>{rowData.first_name || '-'} {rowData.last_name || '-'}</span>
                                )} />

                                <Column field="order id" sortable header="Order Id" style={{ minWidth: '12rem' }} />

                                <Column field="name" sortable header="Customer Name" style={{ minWidth: '12rem' }} body={(rowData) => (
                                    <span>{rowData.country_code || '-'} {rowData.phone_number || '-'}</span>
                                )} />

                                <Column field="balance" sortable header="Balance" style={{ minWidth: '12rem' }} body={(rowData) => (
                                ...
                                <Column field="status" header="Action" style={{ minWidth: '8rem' }} body={(rowData) => (
                                    <div className="action-btn">
                                        <Link to={'/customer_list/customer_details'} state={rowData} className="text-info edit cursor_pointer">
                            ...
                            <div className=''>
                                <Pagination per_page={Constatnt.per_page} pageCount={customerList?.totalCount} onPageChange={onPageChange} page={page} />
                            </div>
                        </div>
                    </div>
                </div>
