...
import { EditCustomer, addCity, customerList, deleteCity, editCity } from '../../utils/api.services';
import { ExportToCSV, ExportToExcel, ExportToPdf, TOAST_ERROR, TOAST_SUCCESS } from '../../config/common';
import { DataTable } from 'primereact/datatable';
import { Column } from 'primereact/column';
import "primereact/resources/themes/lara-light-cyan/theme.css";
import { getCityListThunk, getUserListThunk, setLoader, updateCityList, updateCustomerList } from '../../Store/slices/MasterSlice';
...
export default function ManageCity() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                await dispatch(getCityListThunk(request));
            } finally {
    ...
    const handleDelete = (is_true) => {
        if (is_true) {
            ...
            let submitData = {
                city_id: selectedUser?.id,
            }
            deleteCity(submitData).then((response) => {
                if (response.code === Codes?.SUCCESS) {
                    const updatedList = cityList?.result?.filter((item) => item.id !== selectedUser?.id);
    ...
    const onSubmitData = (data) => {
        ...
        if (is_Edit) {
            sendRequest.city_id = editData?.id;
            editCity(sendRequest).then((response) => {
                if (response?.code === Codes.SUCCESS) {
                    TOAST_SUCCESS(response?.message);
                    setCityModel(false);
                    reset();
            ...
            addCity(sendRequest).then((response) => {
                if (response?.code === Codes.SUCCESS) {
                    TOAST_SUCCESS(response?.message);
                    setCityModel(false);
                    reset();
    ...
    return (
        <>
            <div className="container-fluid mw-100">
                <SubNavbar title={"City List"} header={'City List'} />
                <div className="widget-content searchable-container list">
                    <div className="card card-body">
                        <div className="row">
                            <div className="col-md-4 col-xl-3">
                                <div className="position-relative">
                                    <input type="text" className="form-control product-search ps-5" id="input-search" placeholder="Search City ..."
                            ...
                            <div className="col-md-8 col-xl-9 text-end d-flex justify-content-md-end justify-content-center mt-3 mt-md-0">
                                ...
                                <div id="btn-add-contact" className="btn btn-info d-flex align-items-center" style={{ height: '40px' }} onClick={() => { setCityModel(true) }}>
                                    <i className="ti ti-category me-1 fs-6" />Add City
                                </div>
                    ...
                    <div className="card card-body">
                        <div className="table-responsive">
                            <DataTable
                                ...
                                loading={loading}
                                emptyMessage={<span style={{ textAlign: 'center', display: 'block' }}>No City found.</span>}
                            >
                                <Column
                                    field="id"
                                    header="Id"
                                ...
                                <Column field="name" sortable header="City Name" style={{ minWidth: '10rem', whiteSpace: 'nowrap' }} body={(rowData) => (
                                ...
                                <Column field="status" header="Action" style={{ minWidth: '10rem' }} body={(rowData) => (
                                    <div className="action-btn">
                                        <a className="text-info edit cursor_pointer cursor_pointer me-1" onClick={() => { setValue('city_name', rowData?.name); setIs_Edit(true); setEditData(rowData); setCityModel(true) }} >
                            ...
                            <div className=''>
                                <Pagination per_page={cityList?.perPage} pageCount={cityList?.page_count} onPageChange={onPageChange} page={cityList?.currentPage} />
                            </div>
                ...
                <div className="modal-dialog modal-md modal-dialog-centered" role="document" >
                    <div className="modal-content border-0">
                        <div className="modal-header bg-primary " style={{ borderRadius: '10px 10px 0px 0px' }}>
                            <h6 className="modal-title text-white"> {is_Edit ? 'Edit' : 'Add'} City</h6>
                            <button type="button" className="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close" onClick={() => { setCityModel(false); setIs_Edit(false); setEditData(null); reset() }} />
                        </div>
                                ...
                                <div className="col-lg-12">
                                    <div className="card-body p-4">
                                        <div className='row d-flex gap-3'>
                                            <div className='col'>
                                                <div className="mb-4">
                                                    <label htmlFor="question" className="form-label fw-semibold">City Name<span className="text-danger ms-1"> *</span></label>
                                                    <input type="text" className="form-control ps-2" placeholder="Enter city name" {...register('city_name', { required: "Enter city name" })} />
                                                    <label className="errorc ps-1 pt-1">{errors.city_name?.message}</label>
                                                </div>
                                            </div>
                                        </div>
