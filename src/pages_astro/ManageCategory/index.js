...
import { addCategory, categoryList, categoryListExport, DeleteCategory, EditCategory, editCategory, exportCategoryList, ListCategory } from '../../utils/api.services';
...
import { getcategoryListThunk, setLoader, updateCategoryList } from '../../Store/slices/MasterSlice';
...
export default function WorksheetAnswer() {
    ...
    const { categoryList: { data: categoryListt }, } = useSelector((state) => state.masterslice);
    ...
    const handleDelete = (is_true) => {
        if (is_true) {
            ...
            DeleteCategory(submitData).then((response) => {
                if (response.code === Codes?.SUCCESS) {
                    const updatedList = categoryListt?.CategoryList?.filter((item) => item._id !== selctedCategory?._id)
    ...
    console.log('categoryListt', categoryListt);
    ...
    return (
        <>
            ...
            <div className="container-fluid mw-100">
                ...
                <SubNavbar title={" Category List"} header={' Category List'} />
                <div className="widget-content searchable-container list m-3">
                        ...
                        <div className="row">
                            <div className="col-md-4 col-xl-3">
                                <div className="position-relative">
                                    <input type="text" className="form-control product-search ps-5" id="input-search" placeholder="Search Category..."
                            ...
                            <div className="col-md-8 col-xl-9 text-end d-flex justify-content-md-end justify-content-center mt-3 mt-md-0">
                                ...
                                <Link to={'/category_list/add_category'} id="btn-add-contact" className="btn btn-info d-flex align-items-center" style={{ height: '40px' }}>
                                    <i className="ti ti-category me-1 fs-6" />Add Category
                                </Link>
                    ...
                    <div className="card card-body">
                        <div className="table-responsive">
                            <DataTable
                                value={categoryListt?.CategoryList}
                                // paginator
                                rows={10}
                            ...
                            >
                                    ...
                                    body={(rowData) => (
                                        <div style={{ display: 'flex', justifyContent: 'start', alignItems: 'start' }}>
                                            <img
                                                ...
                                                alt="Profile"
                                                height={50}
                                                width={50} // Ensures a consistent aspect ratio
                                                style={{
                                                    // objectFit: 'cover',
                                ...
                                <Column field="name" sortable header="English Name" style={{ minWidth: '12rem', whiteSpace: 'nowrap' }} body={(rowData) => (
                                    <span>{rowData.name || '-'}</span>
                                )} />

                                <Column field="hi_name" sortable header="Hindi Name" style={{ minWidth: '12rem', whiteSpace: 'nowrap' }} body={(rowData) => (
                                    <span>{rowData.hi_name || '-'}</span>
                                )} />

                                <Column field="gu_name" sortable header="Gujarati Name" style={{ minWidth: '12rem', whiteSpace: 'nowrap' }} body={(rowData) => (
                                    <span>{rowData.gu_name || '-'}</span>
                                )} />

                                <Column field="sequence" sortable header="Position" style={{ minWidth: '12rem', whiteSpace: 'nowrap' }} body={(rowData) => (
                            ...
                            <div className=''>
                                <Pagination per_page={categoryListt?.perPage} pageCount={categoryListt?.totalCategory} onPageChange={onPageChange} page={categoryListt?.currentPage} />
                            </div>
